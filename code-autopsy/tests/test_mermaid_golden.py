...
from xray_core import (  # noqa: E402
    render_er_mermaid,
    render_er_plantuml,
    render_service_architecture_mermaid,
    render_service_architecture_plantuml,
)
...
def test_er_plantuml_contains_entities_and_relations() -> None:
    entities = [
        {
            "name": "User",
            "fields": [{"name": "id", "type": "int"}, {"name": "team_id", "type": "int"}],
    ...
    plantuml = render_er_plantuml(entities, relationships)

    assert plantuml.startswith("@startuml")
    assert 'class "User"' in plantuml
    assert 'class "Team"' in plantuml
    assert "fk" in plantuml
    assert plantuml.rstrip().endswith("@enduml")


def test_service_architecture_plantuml_includes_key_service_modules() -> None:
    parsed_files = [
        {"path": "api/server.py"},
        {"path": "api/auth/handlers.py"},
        {"path": "api/orders/service.py"},
    ...
    plantuml = render_service_architecture_plantuml(
        parsed_files=parsed_files,
        edges=edges,
        frameworks=[],
        routes=routes,
    ...
    assert plantuml.startswith("@startuml")
    assert "API Service (3) - key:" in plantuml
    assert "api/server.py" in plantuml
    assert plantuml.rstrip().endswith("@enduml")
